    ...
    <div className="flex flex-col gap-16 animate-fade-in max-w-6xl mx-auto py-12 pb-48 items-center">
      ...
      <div className="w-full flex flex-col items-center text-center space-y-8 relative px-6">
        ...
        <div className="space-y-6 max-w-4xl">
          ...
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-foreground tracking-tighter leading-tight">
            基于web可视化技术足球科普系统<br/>
            <span className="text-blue-500 text-2xl md:text-3xl lg:text-4xl mt-4 block">——面向球迷的战术平台</span>
          </h2>
          <p className="text-xl text-slate-500 dark:text-gray-400 font-medium leading-relaxed italic max-w-3xl mx-auto opacity-90">
            “ 本系统致力于通过数字化交互，将职业绿茵场上的战术博弈显性化，为数据驱动型球迷提供深度的比赛逻辑拆解。”
          </p>

          {/* 申报与导师信息 */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 w-full mt-12">
            <div className="p-6 bg-foreground/[0.02] border border-border rounded-3xl hover:bg-foreground/[0.04] transition-colors">
                <p className="text-[10px] text-blue-500 font-black uppercase tracking-widest mb-2 opacity-60">研究方向</p>
                <p className="text-lg font-black text-foreground">Web 数据可视化交互</p>
            </div>
            <div className="p-6 bg-foreground/[0.02] border border-border rounded-3xl hover:bg-foreground/[0.04] transition-colors">
                <p className="text-[10px] text-blue-500 font-black uppercase tracking-widest mb-2 opacity-60">课题负责人</p>
                <p className="text-lg font-black text-foreground uppercase">谢坤 / Xie Kun</p>
            </div>
            <div className="p-6 bg-foreground/[0.02] border border-border rounded-3xl hover:bg-foreground/[0.04] transition-colors">
                <p className="text-[10px] text-blue-500 font-black uppercase tracking-widest mb-2 opacity-60">指导教师</p>
                <p className="text-lg font-black text-foreground">周书臣 副教授</p>
            </div>
            <div className="p-6 bg-blue-600/10 border border-blue-500/20 rounded-3xl">
                ...
                <p className="text-lg font-black text-foreground dark:text-white font-mono tracking-widest">v3.2.0-QUANTUM</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full space-y-8 px-6 mt-12">
          <div className="flex items-center gap-4">
              ...
              <h3 className="text-2xl font-black text-foreground uppercase tracking-widest">技术架构与核心创新 / ARCHITECTURE</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {coreInnovations.map((item, i) => (
                <div key={i} className="bg-card border border-border rounded-[2.5rem] p-10 hover:border-blue-500/40 transition-all shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="flex justify-between items-start mb-6 relative z-10">
                        <h4 className="text-xl font-black text-foreground leading-tight uppercase">{item.label}</h4>
                    </div>
                    <div className="flex flex-wrap gap-2 mb-6 relative z-10">
                        {item.techs.map(t => <span key={t} className="px-2.5 py-1 bg-blue-600/10 rounded-lg text-[9px] font-black text-blue-500 dark:text-blue-400 border border-blue-500/20">{t}</span>)}
                    </div>
                    <p className="text-sm text-slate-500 dark:text-gray-400 leading-relaxed font-medium relative z-10">{item.desc}</p>
                </div>
            ))}
          </div>
      </div>
      ...
      <div className="w-full grid grid-cols-1 md:grid-cols-2 gap-12 px-6 mt-12">
        <div className="space-y-8 bg-foreground/[0.02] border border-border rounded-[3rem] p-12 relative overflow-hidden">
            ...
            <div className="space-y-6">
                {academicValue.map((item, i) => (
                    <div key={i} className="space-y-2">
                        <p className="text-lg font-black text-slate-700 dark:text-gray-200 uppercase tracking-tight">{item.title}</p>
                        <p className="text-sm text-slate-500 dark:text-gray-400 leading-relaxed italic">{item.desc}</p>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="space-y-8 bg-blue-600/5 border border-blue-500/10 rounded-[3rem] p-12">
            <h3 className="text-2xl font-black text-blue-500 dark:text-blue-400 uppercase tracking-widest flex items-center gap-4">
                <div className="w-1 h-6 bg-blue-400 rounded-full"></div> 未来展望
            </h3>
            <p className="text-base text-slate-600 dark:text-gray-300 leading-relaxed italic pr-4">
            ...
            <div className="flex gap-4 pt-4 border-t border-border mt-4">
                <span className="px-3 py-1 bg-foreground/5 rounded-full text-[9px] font-black text-slate-500 dark:text-gray-500 uppercase tracking-widest">#BIG_DATA</span>
                <span className="px-3 py-1 bg-foreground/5 rounded-full text-[9px] font-black text-slate-500 dark:text-gray-500 uppercase tracking-widest">#AI_TACTICS</span>
            </div>
        </div>
      </div>
      ...
      <div className="w-full flex flex-col items-center gap-8 mt-24 opacity-40">
          <div className="flex items-center gap-4">
              ...
              <div className="px-8 py-3 rounded-full bg-foreground/5 border border-border text-[10px] text-slate-500 dark:text-gray-500 font-black uppercase tracking-[0.4em]">
                 足球可视化科普实验室 (TacticalLab) • 2025 毕业设计作品
              </div>
              <div className="h-px w-20 bg-border"></div>
          </div>
          <p className="text-[9px] font-medium tracking-widest text-slate-500 dark:text-gray-600 uppercase">Designed for Football Enthusiasts by Xie Kun</p>
      </div>
